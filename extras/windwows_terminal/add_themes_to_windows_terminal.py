...
if __name__ == "__main__":
    ...
    with open(settings, "r+", encoding="utf-8") as file:
        print(f"Adding fenbox themes to Windows Terminal settings @ {args.windows_terminal_settings_json} ...")
        data = json.load(file)
        for scheme in create_windows_terminal_themes().values():
            print(f"Adding fenbox-{scheme['name']} to Windows Terminal settings ...")
